...
if int(args.ept)==1:
    print('using trained EPT model for inference')
    tokenizer = AutoTokenizer.from_pretrained("/cos_mount/users/dibyanayan/starcoder_full_ept/checkpoint-2000")
    model = AutoModelForCausalLM.from_pretrained("/cos_mount/users/dibyanayan/starcoder_full_ept/checkpoint-2000", device_map="auto", torch_dtype=torch.bfloat16)
else:
    print('using original pretrained model for inference')
